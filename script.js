...
function parseBoard(text){
  ...
  const boardLines = lines.filter(l=>l.startsWith("|"));
...
async function readBoard(){
  ...
  const sente = boardPieces
    .filter(p=>p.side==="sente")
    .sort(boardSort);

  for(const p of sente){
    await speak(formatBoardYomi(p));
  }
}
...
function formatBoardYomi(p){
  ...
  const rank = kanjiRankYomi[rankKanji];
...
function drawPieces(){
  ...
  boardPieces.forEach(p=>{
    ...
    const index = (p.rank-1)*9 + (9-p.file);
...
function convertMoveToYomi(move){
  ...
  for(const k in numberYomi){
    result = result.replaceAll(k, numberYomi[k]);
  }

  for(const k in pieceYomi){
    result = result.replaceAll(k, pieceYomi[k]);
  }
